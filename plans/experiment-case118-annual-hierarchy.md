...
| S4b (open; immutable shard manifest reviewed) | bounded qualification plus annual partition | Validate scheduler-neutral sharding and select annual shard boundaries from the frozen S4 outer trajectory | Characterize the partition-boundary effect, then pass partitioned-policy process equivalence, fresh-process execution, deterministic merge/audit, and the bounded two-worker demonstration before S5; execution remains unauthorized |

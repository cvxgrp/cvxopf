...
value-match; see [[case9-dcline-optima-gap]]); **815 tests pass**. The 6b
value-match was ruled out by the `case9_dcline` cross-eval (Pypower's fixture
point is suboptimal for its own problem, not an alternate optimum); the
investigation is distilled and committed as the final report
`experiments/dnlp_vs_pypower/` (2x2 PWL-cost × DC-line study, `REPORT.tex`/`.pdf`
+ reproducible `demo.py`). See the Gate 6b as-built note below. **Only Step 7
...
**Gate 6b (AS-BUILT: `tests/test_hvdc.py::TestHVDCCase9DclineConsistency`, internal-consistency -- NOT a Pypower value-match).** The plan originally scoped 6b as an approximate Pypower oracle (`TestHVDCPypowerApprox`, options (a)/(b)/(c) below). **That design was abandoned:** the 2026-07-14..20 `case9_dcline` cross-eval investigation (`experiments/dcline_crosseval/`, EX6..EX12) established that a solved cvxopf `formulation="ac"` run on `case9_dcline()` **cannot** value-match the committed fixture, and *why* -- the two land in different local optima of the (near-)same nonconvex AC-OPF, and Pypower's fixture point **P\*** is a **suboptimal** point of its own feasible set, not an alternate global optimum. EX12 fired the QED: grafting Pypower's with-`loss0` law onto cvxopf's neutralized problem re-solves to a point that is fully Pypower-feasible **and** cheaper (5469 vs P\* 6249.87, margin 780), staying in cvxopf's basin. Because there is no value-match to assert, 6b became **consistency-based**: nodal balance ≈ 0, the `p_out = -(1-loss_frac)*p_in` law on fixed-direction links, `hvdc_loss >= 0`, and the `loss0` `UserWarning`. Full rationale: [[case9-dcline-optima-gap]].

**Capstone (2026-07-20): `experiments/dnlp_vs_pypower/` (committed).** A controlled 2x2 study (PWL generator costs × HVDC DC lines, DNLP/cvxopf vs standard/Pypower on the *same* problem) distills the investigation into a written report (`REPORT.tex`/`.pdf`) plus a runnable proof-by-code (`demo.py`, reproduces every headline number against committed `reference_pypower.json`). Finding, narrowly scoped: each feature alone -> the two methods agree (base case and PWL-only match to 1e-4 as committed oracle tests); both together -> the standard method's objective is ~14% higher, and the DNLP solution is verifiably feasible in the standard method's own problem. Stated limits: both problems nonconvex, no global-optimality certificate, one network at four operating points. The report is deliberately careful to say DC lines add *routing freedom* (they flatten the landscape) rather than nondifferentiable corners, and does **not** claim the mechanism (reformulation vs solver) is isolated. See [[dnlp-canonicalization-tractability-thesis]].

The original approximate-oracle options are retained here only as the historical record of what was tried before the cross-eval ruled a value-match out:
...
- (c) internal-consistency assertions (flow conservation + the `p_out = -(1-loss1)*p_in` loss law on the fixed-direction links).

Option (c) is what shipped; (a)/(b) were shown to be unachievable because P\* is suboptimal for Pypower's own problem (the objective gap is not a modeling gap to be absorbed by a tolerance -- it is a different local optimum). This gate depends on both AC (Step 5) and DC (Step 4) integration and runs just before the full-suite gate.
...
- `tests/`: add a **Ybus-agreement test** for a dcline case (`case9_dcline`): cvxopf's `build.data["Ybus"]` must equal Pypower's `makeYbus` output for the same case, matched by external bus ID. Rationale: this pins the load-bearing HVDC assumption that **DC lines contribute nothing to Ybus** (they are modelled as injections, not admittance branches) -- verified empirically 2026-07-14 in the dcline cross-eval investigation (`experiments/dcline_crosseval/_ybus_compare.py`, max abs diff 4.4e-16). Per the no-pypower-dependency rule, do **not** call `makeYbus` live in `tests/`; instead commit Pypower's case9_dcline Ybus as a static fixture in `tests/fixtures/` (generate once via `scripts/generate_pypower_fixtures.py` in the isolated numpy-2.2.6 env) and compare cvxopf's Ybus against the fixture. Follows the existing committed-fixture pattern.

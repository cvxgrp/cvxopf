...
`p_nd_t^2 + q_nd_t^2 <= S_max^2` intersected with `0 <= p_nd_t <= R_t`.
DC formulations retain both real-power bounds, `0 <= p_nd_t <= R_t` and
`p_nd_t <= S_max`; whichever upper bound is smaller is active.

...
**Status: the selected one-window comparison is complete and independently reviewed.**
See [AC dispatch adjustments](../experiments/case118_counterfactual/ac_dispatch_adjustments/REPORT.md).
The design below retains the original comparison definitions and broader
proposals; those proposals do not imply that additional windows were run.
...
[analysis design](case118-toy-ac-analysis-design.md). The exact window, tolerances,
initializations, and compute budget used for the completed comparison are in
its [launch protocol](../experiments/case118_counterfactual/ac_dispatch_adjustments/protocol.json).
Preserve the original toy run unchanged; further solves require separate authorization.

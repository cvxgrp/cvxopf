...
| 19 — First-class loads and explicit load shedding | 🚧 In progress | Stages 0–3 complete: fixed active/reactive withdrawals now use the shared device architecture, with MATPOWER conversion and identity-aligned explicit time series. Remaining stages activate the affine served-fraction feasible set, sufficiently large linear value-of-lost-load cost in the same single solve, proportional reactive relief, energy-not-served results, and exact-penalty phase-transition tests. No lexicographic or feasibility-restoration solve. See `plans/milestone-19-load-shedding.md`. |

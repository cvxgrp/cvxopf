...
| 19 — First-class loads and explicit load shedding | 🔲 Future | Move fixed active/reactive withdrawals into the shared device architecture, with MATPOWER conversion and identity-aligned time series. Selected loads gain an affine served-fraction feasible set, a sufficiently large linear value-of-lost-load cost in the same single solve, proportional reactive relief, energy-not-served results, and exact-penalty phase-transition tests. No lexicographic or feasibility-restoration solve. See `plans/milestone-19-load-shedding.md`. |

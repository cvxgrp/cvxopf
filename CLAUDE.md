...
| 19 — First-class loads and explicit load shedding | 🚧 In progress | Stages 0–4 complete: fixed active/reactive withdrawals use the shared device architecture, with MATPOWER conversion and identity-aligned explicit time series; configured loads add an affine served-fraction feasible set, proportional reactive relief, and a sufficiently large linear value-of-lost-load cost in the same single solve. Remaining work covers result extraction and unsuccessful-solve behavior, scientific phase-transition and network verification, examples, and documentation. No lexicographic or feasibility-restoration solve. See `plans/milestone-19-load-shedding.md`. |

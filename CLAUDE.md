...
| 14 — Time-vectorized multistep formulations | 🟧 M14c pre-annual profiling | M14a's frozen baseline, M14a.1/M14b's formulation-specific leaf decisions, M14c's explicit vectorized lossy-DC builder, integration, and the supervised vectorized 24/168/720 Case118 prefix ladder are complete. Before annual authority is updated, the identical prefixes must be characterized through the retained `stepwise + CPP` production path; performance is descriptive, while a mathematical mismatch blocks progression. AC permits no new leaf-bound migration. See `plans/milestone-14-time-vectorization.md`, `experiments/m14_time_vectorization/M14B_PROTOCOL.md`, `experiments/m14_time_vectorization/M14C_PROTOCOL.md`, and `experiments/m14_time_vectorization/M14C_PROFILING_PROTOCOL.md`. |

...
| 14 — Time-vectorized multistep formulations | 🟧 M14a–c complete; M14d remaining | The explicit vectorized lossy-DC builder, conditioned 24/168/720 prefix ladder, representation comparison and certificate-backed disposition, and accepted 8,760-hour Case118 S4 solve are complete and merged into `main`. M14d remains: single-node DC and AC vectorization, reusing the Case118 hierarchical initialization helpers, with correctness and hierarchy regression tests. Initial Case9 Tracy comparisons use T=3 and T=168 for stepwise/vectorized DC and AC, reaching 168 incrementally. Measure construction, canonicalization, solver, extraction, total time, peak memory, graph size, and numerical outcomes. The intervention is vectorization; DCP uses the appropriate CPP/SCIPY backend and AC retains DNLP/IPOPT in both forms. No speedup threshold is required. The stepwise default remains; AC permits no new leaf-bound migration. See `plans/milestone-14-time-vectorization.md` and `experiments/m14_time_vectorization/M14C_PROTOCOL.md`. |
